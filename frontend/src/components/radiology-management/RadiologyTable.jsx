...
export default function RadiologyTable({ requests, onUploadReport, onViewReport }) {
  ...
  if (requests.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-10 text-center text-gray-400 text-sm">
        No radiology requests match your filters.
      </div>
    );
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 text-xs border-b border-gray-100 bg-gray-50">
              <th className="px-4 py-3 font-medium">Radiology ID</th>
            ...
            {requests.map((r) => (
              <tr key={r.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3">
                  <p className="font-medium text-gray-800">{r.testType}</p>
                  <p className="text-xs text-gray-400">{r.bodyPart}</p>
                </td>
                <td className="px-4 py-3 text-gray-600">{r.patient}</td>
                <td className="px-4 py-3 text-gray-600">{r.requestedBy}</td>
                <td className="px-4 py-3 text-gray-600">₹{r.price}</td>
                <td className="px-4 py-3">
                  ...
                  {menuOpenFor === r.id && (
                    <div ref={menuRef} className="absolute right-4 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-20 text-left">
                      {r.status !== "Report Ready" && (
                        <button onClick={() => { onUploadReport(r); setMenuOpenFor(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50">
